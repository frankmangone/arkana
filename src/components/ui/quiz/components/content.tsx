...
const styles = {
  ...
  borderWrapper:
    "mt-[-1] p-[2px] bg-gradient-to-b from-secondary-700 to-primary-700",
...
export const QuizContent = (props: QuizContentProps) => {
  const { isOpen, questionId, questionType, questionContent, dictionary } =
    props;
    ...
    <div className={`${styles.container} ${isOpen ? "block" : "hidden"}`}>
      <div className={styles.borderWrapper} style={{ borderTop: "none" }}>
        ...
        <QuizResult
          result={result}
          dictionary={dictionary}
          feedback={questionContent.feedback}
        />
      </div>
    </div>
  );
